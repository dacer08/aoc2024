...
namespace AdventOfCode.Pending.D20;

public class D20
{
    private const string INPUT = "D20/InputTest.txt";
    ...
    public void Run()
    {
        ...
        for (var i = 1; i < 20; i++)
        {
            ...
            foreach (var noWallsEnum in Combinations.GetKCombs(defaultWalls, i))
            //for (var i = 0; i < defaultWalls.Count; i++)
            {
                //var noWalls = defaultWalls[i];
                //Vector2D w1 = new Vector2D(1, 5);// noWalls.First();
                ...
                var noWalls = noWallsEnum.ToList();
                foreach (var w1 in noWalls)
                {
                    if (w1.X == 0 || w1.Y == 0 || w1.X == maxX || w1.Y == maxY)
                    {
                        shouldStop = true;
                    ...
                    if (i > 1 && !noWalls.Where(w2 => w2 != w1).Any(w2 => Math.Abs(w1.X - w2.X) == 1 || Math.Abs(w1.Y - w2.Y) == 1))
                    {
                        shouldStop = true;
                        break;
                    }
                ...
                Scores = [];
                //walls = defaultWalls.Where((w, i2) => i != i2).ToHashSet();
                walls = defaultWalls.Except(noWalls).ToHashSet();
